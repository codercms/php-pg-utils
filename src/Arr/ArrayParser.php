...
namespace Erg\PgUtils\Arr;

use InvalidArgumentException;

/**
 * @internal AMPHP's implementation
 *
 * @link https://github.com/amphp/postgres/blob/master/src/Internal/ArrayParser.php
 */
final class ArrayParser
{
    ...
    public function parse(string $data, callable $cast, string $delimiter = ','): array
    {
        ...
        if ($parser->getReturn() !== '') {
            throw new InvalidArgumentException("Data left in buffer after parsing");
        }
    ...
    private function parser(string $data, callable $cast, string $delimiter = ','): \Generator
    {
        ...
        if ($data[0] !== '{') {
            throw new InvalidArgumentException("Missing opening bracket");
        }
        ...
        do {
            ...
            if ($data[0] === '"') { // Quoted value
                ...
                if (!isset($data[$position])) {
                    throw new InvalidArgumentException("Could not find matching quote in quoted value");
                }
